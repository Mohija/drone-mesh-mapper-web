...
const SHOPPING_LISTS: Record<string, { title: string; note: string; items: ShoppingItem[] }> = {
  ...
  'esp32-s3-gps': {
    title: 'ESP32+GPS (mit Standort & RGB-Taster)',
    note: 'Wie ESP32-S3, zusätzlich mit ATGM336H GPS (automatischer Standort) und externem beleuchteten Taster (Ein/Aus per Langdruck, Status-RGB). Mindestens 5 Adern zum Taster (COM+, SW, R–, G–, B–). Pin-Belegung: GPS an UART1 (GPIO 17/18 @ 9600 Baud), Taster an GPIO 4 (INPUT_PULLDOWN), LEDs an GPIO 5/6/7 mit Vorwiderständen 100/100/33 Ω (Common-Anode = LOW schaltet LED ein).',
    items: [
      { name: 'ESP32-S3-DevKitC-1 N16R8 mit IPEX + Antenne', desc: 'Heemol Board: 16 MB Flash, 8 MB PSRAM, WiFi + BLE 5.0, USB-C, Dual-Core 240 MHz. Mit IPEX-Anschluss + 2,4 GHz Antenne im Lieferumfang. Pin-Headers vorgelötet.', price: '~15 €', required: true, group: 'controller-set',
        link: 'https://www.amazon.de/Heemol-DevKitC-1-Entwicklung-Bluetooth-Anschlie%C3%9Fbare/dp/B0FKFXC6F8' },
      { name: 'GPIO Breakout Board für ESP32-S3', desc: 'Steckboden mit Montagelöchern — ESP32 einstecken, im Gehäuse verschrauben. Kein Löten, kein Breadboard. 5V/3.3V Ausgänge, GPIO-Status-LEDs.', price: '~12 €', required: true, group: 'controller-set',
        link: 'https://www.amazon.de/Meshnology-Erweiterungsboard-Kunststoffdichtungen-Steckdosen-N40/dp/B0FLK4MDDW' },
      { name: 'ATGM336H-5N GPS-Modul (GPS + BeiDou, UART)', desc: 'Kompaktes GNSS-Modul mit aktiver Keramikantenne und 3,3 V-Eingang. UART mit 9600 Baud (Standard), liefert NMEA-Sätze (GGA, RMC). Fix-Zeit draußen ca. 30–60 s. Passt auf die Breakout-Leiste; nur 4 Drähte nötig: VCC (3,3 V), GND, TX → GPIO 17, RX → GPIO 18.', price: '~10 €', required: true, group: 'gps-taster',
        link: 'https://www.amazon.de/s?k=ATGM336H+GPS+BeiDou+Modul+UART' },
      { name: 'RGB-Drucktaster 16 mm (1× NO, Common-Anode, Metall)', desc: 'Beleuchteter Edelstahl-Taster mit integrierter RGB-LED und gemeinsamer Anode (COM+ versorgt sowohl Schaltkontakt als auch alle drei LEDs). 1× Schließer (NO), IP65, 12–24 V LED-Betrieb (läuft an 3,3 V dimm-fähig). Einbaubohrung Ø 16 mm.', price: '~10 €', required: true, group: 'gps-taster',
        link: 'https://www.amazon.de/s?k=16mm+RGB+Drucktaster+common+anode+edelstahl' },
      { name: 'Metallfilm-Widerstände Sortiment (100 Ω + 33 Ω)', desc: 'Für die drei LED-Kathoden: 2× 100 Ω (rot + grün) und 1× 33 Ω (blau, braucht mehr Strom). Sortiment-Boxen enthalten alle drei Werte; alternativ Einzel­widerstände ¼ W ausreichend.', price: '~8 €', required: true, group: 'gps-taster',
        link: 'https://www.amazon.de/s?k=widerstand+sortiment+1%2F4W+metallschicht' },
      { name: 'Dupont-Jumper-Kabel Set (Female-Female, 20 cm)', desc: 'Für die Verbindungen zwischen Breakout-Board, GPS-Modul und Taster. Mindestens 9 Leitungen nötig (4 für GPS, 5 für den Taster). Ein 40er-Bundel reicht weit.', price: '~6 €', required: true,
        link: 'https://www.amazon.de/s?k=Dupont+Jumper+Kabel+female+female+20cm' },
      { name: 'USB-A auf USB-C Kabel (1m)', desc: 'Datenkabel (nicht nur Lade!) für Flashen und Stromversorgung.', price: '~7 €', required: true,
        link: 'https://www.amazon.de/1-m-langes-usb-c-kabel-usb-a-auf-usb-c-von-amazon/dp/B07Q5JW4J3' },
      { name: 'USB-Netzteil 5V/2A (USB-C)', desc: 'Steckernetzteil für Dauerbetrieb. 5V, min. 1A (2A empfohlen).', price: '~8 €', required: true,
        link: 'https://www.amazon.de/Bouge-Universal-Ladeger%C3%A4t-Kompatibilit%C3%A4t-Blackview/dp/B0C2Q5LK11' },
      { name: 'ABS-Gehäuse IP65 (100×68×50 mm) mit Tasterausschnitt', desc: 'Wasserdichtes Elektronik-Gehäuse. Board + Breakout + GPS-Modul passen hinein; der 16-mm-Taster wird in den Deckel eingelassen (Bohrung Ø 16 mm). Antenne + USB durch Kabelverschraubungen nach außen.', price: '~8 €', required: true,
        link: 'https://www.amazon.de/Elektronische-Wasserdichte-Industriegeh%C3%A4use-Anschlussdose-Verteilerdose/dp/B0DDWR9LP3' },
      { name: 'Kabelverschraubung M16 IP68 (5er-Pack)', desc: 'Wasserdichte Kabel-Durchführung für USB-Kabel, Antenne und — wenn der Taster aus dem Gehäuse herausgeführt wird — für das Taster-Kabel. Alternativ den Taster direkt im Deckel einbauen.', price: '~7 €', required: false,
        link: 'https://www.amazon.de/Kabelverschraubung-M16-Hanibos-Kabeldurchf%C3%BChrung-Kabelverschraubungen/dp/B0BXRVX368' },
      { name: 'PoE-Splitter 5V USB-C (IEEE 802.3af)', desc: 'Stromversorgung über Ethernet-Kabel. Spart extra Stromkabel bei Outdoor-Installation. GPS-Empfang braucht freien Himmel — daher oft in Dachhöhe montiert.', price: '~15 €', required: false,
        link: 'https://www.amazon.de/UCTRONICS-PoE-Splitter-USB-C-USB-C-Adapter-Sicherheitskameras/dp/B087F4QCTR' },
    ],
  },
};
