...
async function loginAsUser(page: import('@playwright/test').Page, username: string, password = PASSWORD) {
  // Clear pre-loaded admin auth state so the login page appears
  await page.goto('/');
...
test.describe('Multi-Tenant Setup', () => {
  ...
  test('create test tenants and users', async ({ request }) => {
    ...
    const tB = await resB.json();
    tenantBId = tB.id;

    // Create User A1 in Tenant A
    const resUA1 = await request.post('/api/admin/users', {
      headers: adminHeaders,
      data: {
        username: USER_A1, email: `${USER_A1}@test.local`, password: PASSWORD,
        display_name: 'User A1', role: 'user', tenant_id: tenantAId,
      },
    });
    expect(resUA1.status()).toBe(201);
    userA1Id = (await resUA1.json()).id;

    // Create User A2 in Tenant A
    const resUA2 = await request.post('/api/admin/users', {
      headers: adminHeaders,
      data: {
        username: USER_A2, email: `${USER_A2}@test.local`, password: PASSWORD,
        display_name: 'User A2', role: 'user', tenant_id: tenantAId,
      },
    });
    expect(resUA2.status()).toBe(201);
    userA2Id = (await resUA2.json()).id;

    // Create User B1 in Tenant B
    const resUB1 = await request.post('/api/admin/users', {
      headers: adminHeaders,
      data: {
        username: USER_B1, email: `${USER_B1}@test.local`, password: PASSWORD,
        display_name: 'User B1', role: 'user', tenant_id: tenantBId,
      },
    });
    expect(resUB1.status()).toBe(201);
    userB1Id = (await resUB1.json()).id;

    // Create tenant_admin in Tenant A
    const resTAdmin = await request.post('/api/admin/users', {
      headers: adminHeaders,
      data: {
        username: TENANT_ADMIN_A, email: `${TENANT_ADMIN_A}@test.local`, password: PASSWORD,
        display_name: 'Tenant Admin A', role: 'tenant_admin', tenant_id: tenantAId,
      },
    });
    expect(resTAdmin.status()).toBe(201);
    tenantAdminAId = (await resTAdmin.json()).id;
  });
});
...
test.describe('Zone Tenant Isolation (API)', () => {
  ...
  test.beforeAll(async ({ request }) => {
    if (!adminHeaders) adminHeaders = await apiLogin(request);
  ...
  test.afterAll(async ({ request }) => {
    for (const headers of [headersA1, headersB1]) {
      if (!headers) continue;
  ...
  test('User A1 creates a zone in Tenant A', async ({ request }) => {
  ...
  test('User A2 (same tenant) sees the zone', async ({ request }) => {
  ...
  test('User B1 creates a zone in Tenant B', async ({ request }) => {
    const res = await request.post('/api/zones', {
      headers: headersB1,
      data: {
  ...
  test('User B1 cannot update Tenant A zone', async ({ request }) => {
  ...
  test('User B1 cannot delete Tenant A zone', async ({ request }) => {
    const res = await request.delete(`/api/zones/${zoneAId}`, { headers: headersB1 });
    expect(res.status()).toBe(404);

    // Verify zone still exists for User A1
    const check = await request.get(`/api/zones/${zoneAId}`, { headers: headersA1 });
    expect(check.status()).toBe(200);
  });

  test('User A2 can update zone created by User A1 (same tenant)', async ({ request }) => {
    const res = await request.put(`/api/zones/${zoneAId}`, {
      headers: headersA2,
  ...
  test('User A2 can delete zone created by User A1 (same tenant)', async ({ request }) => {
    ...
    const res = await request.delete(`/api/zones/${zone.id}`, { headers: headersA2 });
    expect(res.status()).toBe(200);
  });

  test('User B1 cannot assign drones to Tenant A zone', async ({ request }) => {
...
test.describe('Violation Tenant Isolation (API)', () => {
  ...
  test.afterAll(async ({ request }) => {
    if (bigZoneAId && headersA1) {
      await request.delete(`/api/zones/${bigZoneAId}`, { headers: headersA1 });
    }
  });
  ...
  test('Tenant A sees violations for its zones', async ({ request }) => {
    ...
    for (const v of data.violations) {
      expect(v.zoneId).toBe(bigZoneAId);
    }
  });
...
test.describe('Role-Based Access Control', () => {
  ...
  test('regular user cannot create users', async ({ request }) => {
    const res = await request.post('/api/admin/users', {
      ...
      data: {
        ...
        password: 'hack123', role: 'user', tenant_id: tenantAId,
      },
    });
    expect(res.status()).toBe(403);
  });
  ...
  test('tenant_admin cannot delete tenants', async ({ request }) => {
    const res = await request.delete(`/api/admin/tenants/${tenantBId}`, {
      headers: headersTAdminA,
    });
    expect(res.status()).toBe(403);
  });
...
test.describe('Admin Board: Tenant Details', () => {
  ...
  test('tenant list shows user and zone counts', async ({ request }) => {
    ...
    expect(tA.user_count).toBeGreaterThanOrEqual(3); // A1, A2, tenant_admin
  ...
  test('super_admin can filter users by tenant', async ({ request }) => {
    const res = await request.get(`/api/admin/users?tenant_id=${tenantBId}`, {
      headers: adminHeaders,
    });
    const users = await res.json();
    for (const u of users) {
  ...
  test('super_admin can change user password', async ({ request }) => {
    const res = await request.post(`/api/admin/users/${userA1Id}/password`, {
      headers: adminHeaders,
    ...
    // Reset password back
    await request.post(`/api/admin/users/${userA1Id}/password`, {
      headers: adminHeaders,
...
test.describe('Same Tenant UI Isolation', () => {
  ...
  test.afterAll(async ({ request }) => {
    if (uiZoneId && headersA1) {
      await request.delete(`/api/zones/${uiZoneId}`, { headers: headersA1 });
    }
  });

  test('User A2 sees zone created by A1 in the UI', async ({ page }) => {
...
test.describe('Same Tenant Shared Violations', () => {
  ...
  test.afterAll(async ({ request }) => {
    if (violationZoneId && headersA1) {
      await request.delete(`/api/zones/${violationZoneId}`, { headers: headersA1 });
    }
  });
...
test.describe('Multi-Tenant Cleanup', () => {
  test('cleanup test data', async ({ request }) => {
    ...
    // Delete test tenants (cascade deletes users and zones)
    const tenantsRes = await request.get('/api/admin/tenants', { headers });
    const tenants = await tenantsRes.json();
    for (const t of tenants) {
      if (t.name.startsWith('e2e-') && t.name.includes(uid)) {
        await request.delete(`/api/admin/tenants/${t.id}`, { headers });
      }
    }

    // Delete any orphaned e2e users (in case cascade didn't catch them)
    const usersRes = await request.get('/api/admin/users', { headers });
    const users = await usersRes.json();
    for (const u of users) {
      if (u.username.startsWith('e2e-') && u.username.includes(uid)) {
