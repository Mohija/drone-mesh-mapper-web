...
class ReceiverNode(db.Model):
    """Hardware receiver node (ESP32-S3, ESP32-C3, ESP32-S3+GPS) for Open Drone ID detection."""
    __tablename__ = "receiver_nodes"

    HARDWARE_TYPES = ("esp32-s3", "esp32-c3", "esp32-s3-gps")
    ...
    tenant_id = db.Column(db.String(8), db.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False)
    name = db.Column(db.String(200), nullable=False)
    hardware_type = db.Column(db.String(20), nullable=False)  # esp32-s3, esp32-c3, esp32-s3-gps
