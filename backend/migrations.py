...
MIGRATIONS: list[dict[str, Any]] = [
    ...
    {
        "version": "015_alarm_interfaces",
        "description": "Outbound integrations: alarm_interfaces, alarm_rules, alarm_deliveries — webhook push + pull",
        "statements": [
            """CREATE TABLE IF NOT EXISTS alarm_interfaces (
                id VARCHAR(8) PRIMARY KEY,
                tenant_id VARCHAR(8) NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                name VARCHAR(100) NOT NULL,
                description TEXT,
                ...
                retry_backoff_seconds REAL DEFAULT 2.0 NOT NULL,
                auth_type VARCHAR(20) DEFAULT 'none' NOT NULL,
                auth_config_encrypted TEXT,
                pull_interval_seconds INTEGER DEFAULT 60,
                service_token_id VARCHAR(8) REFERENCES service_tokens(id) ON DELETE SET NULL,
                payload_template JSON,
                created_at REAL NOT NULL,
                updated_at REAL NOT NULL,
                created_by VARCHAR(100)
            )""",
            "CREATE INDEX IF NOT EXISTS idx_alarm_interfaces_tenant ON alarm_interfaces(tenant_id)",
            """CREATE TABLE IF NOT EXISTS alarm_rules (
                id VARCHAR(8) PRIMARY KEY,
                tenant_id VARCHAR(8) NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                interface_id VARCHAR(8) NOT NULL REFERENCES alarm_interfaces(id) ON DELETE CASCADE,
                zone_id VARCHAR(8) REFERENCES flight_zones(id) ON DELETE CASCADE,
                name VARCHAR(100),
            ...
            "CREATE INDEX IF NOT EXISTS idx_alarm_rules_interface ON alarm_rules(interface_id)",
            "CREATE INDEX IF NOT EXISTS idx_alarm_rules_zone ON alarm_rules(zone_id)",
            """CREATE TABLE IF NOT EXISTS alarm_deliveries (
                id VARCHAR(8) PRIMARY KEY,
                tenant_id VARCHAR(8) NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                rule_id VARCHAR(8) REFERENCES alarm_rules(id) ON DELETE SET NULL,
                interface_id VARCHAR(8) NOT NULL REFERENCES alarm_interfaces(id) ON DELETE CASCADE,
                violation_id VARCHAR(8),
            ...
            "CREATE INDEX IF NOT EXISTS idx_alarm_deliveries_tenant ON alarm_deliveries(tenant_id)",
            "CREATE INDEX IF NOT EXISTS idx_alarm_deliveries_interface ON alarm_deliveries(interface_id)",
